'''
cd D:/Code/projects/Real-ESRGAN        
$infile="D:/Document/!Resources/fritia/芙提雅-小熊软糖.mp4"   
$infile="D:/Document/!Resources/megumin/megumin-jimeng3.mp4"
$infile="D:/Document/ai-sings/Rera/𝟒𝐊 𝟔𝟎𝐅𝐏𝐒 初音未来 Rera.mp4"
& uv run inference_realesrgan_video_fast.py -n realesr-animevideov3 -i $infile --outscale 2

# 手动指定fps（可超过原视频，有时总会抽取出比原本该有的更多的帧）
& uv run inference_realesrgan_video_fast.py -n realesr-animevideov3 -i $infile --outscale 1.5 --fps 30

# --outscale小于模型固有设置（x4）：先超分，再下采样到目标尺寸
'''
import argparse
import cv2
import glob
...
def get_sub_video(args, num_process, process_idx):
  ...
  cmd = [args.ffmpeg_bin, f'-i {args.input}', '-ss', f'{part_time * process_idx}', f'-to {part_time * (process_idx + 1)}' if process_idx != num_process - 1 else '', '-async 1', out_path, '-y']
  print(' '.join(cmd))
...
class Reader:
  ...
  def __init__(self, args, total_workers=1, worker_idx=0):
    ...
    if self.input_type.startswith('video'):
      ...
      self.stream_reader = ffmpeg.input(video_path).filter('fps', fps=self.get_fps()).output(
      # self.stream_reader = ffmpeg.input(video_path).output(
        'pipe:', format='rawvideo', pix_fmt='rgb24', loglevel='error'
  ...
  def get_frame_from_stream(self):
    img_bytes = self.stream_reader.stdout.read(self.frame_size)  
    if len(img_bytes) < self.frame_size:
      return None
...
class Writer:
  ...
  def __init__(self, args, audio, height, width, video_save_path, fps):
    ...
    if out_height > 2160:
      print('You are generating video that is larger than 4K, which will be very slow due to IO speed.', 'We highly recommend to decrease the outscale(aka, -s).')

    if audio is not None:
      self.stream_writer = ffmpeg.input(
        'pipe:', format='rawvideo', pix_fmt='rgb24', s=f'{out_width}x{out_height}', framerate=fps).output(
          audio, video_save_path, pix_fmt='yuv420p', vcodec='libx264', loglevel='error', acodec='copy'
...
def main():
  ...
  parser.add_argument('-dn', '--denoise_strength', type=float, default=0.5, help=('Denoise strength. 0 for weak denoise (keep noise), 1 for strong denoise ability. '
